#!/usr/bin/env python3
"""
Script de entrenamiento personalizado para ModelM7
Entrena 20 modelos con diferentes porcentajes de datos (5% a 100%)
"""
...
def run_custom(p_seed=0, p_epochs=150, p_data_percentage=100, p_logdir="temp"):
    """
    Función de entrenamiento personalizada
    """
    print(f"\n" + "="*80)
    print(f"🚀 ENTRENANDO MODELO M7 - {p_data_percentage}% DE DATOS - SEED {p_seed}")
    print(f"="*80)
    ...
    OUTPUT_FILE = str("../logs/%s/log_%02d_percent_seed_%03d.out" % (p_logdir, DATA_PERCENTAGE, SEED))
    MODEL_FILE = str("../logs/%s/model_%02d_percent_seed_%03d.pth" % (p_logdir, DATA_PERCENTAGE, SEED))
    ...
    full_train_dataset = MnistDataset(training=True, transform=transform)
    ...
    f.write("# ModelM7 Training Log - Custom Data Percentage\n")
...
def main():
    """Función principal que ejecuta el entrenamiento progresivo"""
    print("="*80)
    print("🧪 ENTRENAMIENTO PERSONALIZADO - MODELM7 CON DATOS PROGRESIVOS")
    print("="*80)
    
    # Configuración
    base_seed = 42
    epochs = 150
    logdir = "modelM7_progressive"
    
    # Lista de porcentajes (5%, 10%, 15%, ..., 100%)
    percentages = list(range(5, 101, 5))  # [5, 10, 15, ..., 100]
    ...
    print(f"   - Directorio de logs: ../logs/{logdir}")
    ...
    summary_file = f"../logs/{logdir}/training_summary.txt"
    with open(summary_file, 'w') as f:
        f.write("# ModelM7 Progressive Training Summary\n")
